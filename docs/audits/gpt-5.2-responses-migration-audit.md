...
- Gaps to address: logging defaults still encourage storing full prompts/responses, the assistant route now runs at temperature `1.0` (docs still say `0.4`), prompt cache key is documented but never sent, and Responses usage metrics omit reasoning tokens/request IDs in structured logs.
...
- **Medium — Runtime vs docs mismatch on temperature (`src/app/api/perazzi-assistant/route.ts`)**: Responses calls use `temperature: 1.0` while the updated API contracts still describe `0.4`. This likely changes tone/variance relative to expectations. Mitigation: align code to the documented value or revise the docs + tests to reflect the higher temperature.
