...
- Global styles + tokens: `src/app/globals.css` (CSS variables, `@theme inline`, utility layers, base styles, keyframes).
...
- **Primary token file**: `src/app/globals.css` defines `:root` and `[data-theme="dark"]` CSS variables, plus Tailwind token bindings in `@theme inline`.
- **Secondary/legacy token files**:
  - `src/lib/tokens.ts` defines colors/spacing/fonts but is not referenced in UI code.
  - `src/app/chat.css` redefines brand tokens and button styles; no imports found.
- **Token duplication**:
...
- **Radii and shadows** live in `src/app/globals.css` (`rounded-*`, `shadow-*`), plus component-local Tailwind classes.

### Typography
- `src/app/layout.tsx` sets Next fonts with CSS variables (`--font-geist-sans`, `--font-geist-mono`, `--font-artisan`).
- `src/app/globals.css` defines `--font-sans`, `--font-serif`, `--font-artisan`, `--font-mono` in `@theme inline` and sets `body` to `var(--font-sans)`.
...
- Rich text relies on `.prose` styles in `src/app/globals.css` via `@tailwindcss/typography`.
...
- CSS motion tokens live in `src/app/globals.css` (`@keyframes accordion-*`, `transition` usage in classnames).
...
- Glass/scrim overlays and cinematic gradients are implemented with repeated inline `style` blocks across multiple components (e.g., heritage, experience, home, shotguns) rather than a shared utility or token.
...
- `src/app/chat.css` duplicates tokens and button styles, but is not imported anywhere.
...
- Unused/legacy classes exist in `src/app/globals.css` (`.btn--primary`, `.button-primary`, `.button-secondary`) and `src/app/chat.css`.

## Consolidation Proposal (Single File Source of Truth)
**Recommendation:** Make `src/app/globals.css` the single canonical aesthetics file and treat it as the only source of truth for tokens, typography scale, motion constants, radius, shadows, and surface patterns.

Suggested consolidation path:
1. **Token unification in `globals.css`**
...
2. **Typography scale in `globals.css`**
...
3. **Motion tokens in `globals.css`**
...
4. **Pattern extraction**
   - Convert repeated scrim/gradient patterns into reusable CSS classes or `@layer utilities` in `globals.css`.
5. **Remove or rewire duplicates**
   - Delete `src/app/chat.css` if unused, or merge its tokens into `globals.css` and ensure chat uses the shared tokens.
   - Remove unused `.btn--primary`/`.button-*` if not referenced.
6. **Documentation alignment**
   - Update `docs/ui-style-contract.md` to explicitly declare `src/app/globals.css` as the single source of truth and describe how to add/modify tokens there.
