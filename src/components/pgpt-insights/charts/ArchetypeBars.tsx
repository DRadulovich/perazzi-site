...
type ArchetypeBarDatum = {
  archetype: string;
  total: number;
  avgMaxScore: number | null;
};

type ArchetypeBarsProps = {
  title?: string;
  subtitle?: string;
  data: ArchetypeBarDatum[];
          ...
          return (
            <div key={row.archetype} className="space-y-1.5">
              ...
              <div className={cn("relative overflow-hidden rounded-xl border border-border/70 bg-muted/30", barHeight)}>
                <div
                  className="absolute inset-y-0 left-0 bg-gradient-to-r from-blue-500/70 via-blue-500/50 to-blue-500/30"
